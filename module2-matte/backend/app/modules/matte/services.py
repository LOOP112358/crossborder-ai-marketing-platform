...
def _mock_remove(image: Image.Image) -> Image.Image:
    """Mock 模式：近白 / 浅灰背景透明化（ABO 商品图常见白底），用 numpy 加速。"""
    ...
    try:
        ...
        spread = rgb.max(axis=2) - whiteness
        mask = (whiteness > 232) & (spread < 28)
        ...
        for r, g, b, a in rgba.getdata():
            ...
            alpha = 0 if whiteness > 232 and spread < 28 else a
...
def remove_background(image_bytes: bytes, edge_smoothing: int = 1) -> Image.Image:
    """默认快速白底抠图（适合 ABO）；仅当 MATTE_USE_REMBG=1 时启用 rembg（很慢）。"""
    image = Image.open(io.BytesIO(image_bytes)).convert("RGBA")
    use_rembg = os.getenv("MATTE_USE_REMBG", "").strip() == "1"
    force_mock = os.getenv("MATTE_MOCK_MODE", "").strip() == "1"
    result = None
    if use_rembg and not force_mock:
        try:
            ...
            print(f"[matte] rembg 失败，回退白底抠图: {e}")
            result = None
    if result is None:
...
async def recognize(image_bytes: bytes, skip_ollama: bool = False) -> dict[str, Any]:
    """商品识别：默认轻量识别；Ollama 可选且易超时，工作流里通常跳过。"""
    image = Image.open(io.BytesIO(image_bytes))
    attrs = lightweight_attributes(image)
    if skip_ollama or os.getenv("MATTE_SKIP_OLLAMA", "1").strip() != "0":
        return {
            "category": "商品",
            "category_en": "product",
            "confidence": 0.7,
